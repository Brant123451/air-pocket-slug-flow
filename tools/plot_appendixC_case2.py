"""Plot LASSI Appendix C case 2 holdup profiles for Fig C.5 comparison."""
...
OUT_DIR = ROOT / "outputs_appendixC_case2"
FIG_DIR = ROOT / "figs_appendixC"
FIG_DIR.mkdir(exist_ok=True)
TAG = "appendixC_case2"
L_PIPE = 20.0
TARGETS = [2.3, 2.4, 2.5, 2.6, 4.0]
...
def main():
    ...
    for ax, tt, (ts, path) in zip(axes, TARGETS, sel):
        ...
        h = np.clip(beta, 0, 1)
        ax.fill_between(x, 0, h, step="pre", color="#1f2e52", lw=0)
    ...
    fig.tight_layout()
    out_png = FIG_DIR / "appendixC_case2_figC5_style.png"
    ...
    for tt, (ts, path) in zip(TARGETS, sel):
        t_val, x, beta = load_curve(path)
        print(f"  t_target={tt:.2f}  t_snap={ts:.3f}  beta_min={beta.min():.4f}  beta_max={beta.max():.4f}  n_sec_or_slug={len(beta)//2}")
