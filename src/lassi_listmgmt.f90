...
contains
   ...
   subroutine list_management(g, dt)
      ...
      call merge_cfl_breaching_sections(g, dt)
      call merge_short_sections(g)
      call purge_zero_length_sections(g)
      ! Paper §3.7.4 + §4.5 ordering: convert any already-flooded
      ! section (β>β_init) into a SLUG BEFORE letting pre-existing
      ! slugs absorb downstream sections via the hydraulic-shock
      ! front.  If slug_front_eat runs first, an existing slug sees
      ! a β≈0.99 film just ahead of its nose; the front velocity
      ! U_F = (Um - β·Ul_film)/(1-β) then diverges as β→1, and the
      ! slug eats tens of metres of pipe in a single δt (this is the
      ! origin of the "mega-slug" artefact in case 4).  Running
      ! slug_init first guarantees every flooded section is either
      ! promoted to its own SLUG (Case A) or merged into an adjacent
      ! SLUG (Case B) with conservative mass-transfer, so the
      ! subsequent slug_front_eat only ever sees β≤β_init sections.
      call slug_init(g)
      call slug_front_eat(g, dt)     ! thesis §4.5: steep-front absorbs sections it has overshot
      if (g%enable_slug_coalescence) call merge_adjacent_slugs(g)   ! optional thesis page 24 coalescence
      ! Paper page 47 "Slug survives?" step: a slug dies if the liquid
      ! it sheds into its tail bubble (nose) exceeds the liquid it eats
      ! from the film ahead within δt.  See slug_shed for judgment.
      call slug_shed(g, dt)
...
!======================================================================
   subroutine merge_cfl_breaching_sections(g, dt)
      type(grid_t), intent(inout) :: g
      real(rk),     intent(in)    :: dt
...
!======================================================================
   subroutine slug_init(g)
      ...
      real(rk) :: kappa_local, U_g_local, rho_g_local, alpha_local
      logical  :: trigger_holdup, trigger_ikh, do_trigger
      integer  :: ntry
      ...
      do while (associated(p))
         p_next => p%next
         ! Two slug-onset criteria, matching the thesis algorithm.
         !
         ! (a) Holdup-based (thesis §3.7.4, page 48): "Whenever the
         !     holdup within a given section exceeds a critical value
         !     βinit the section is removed and replaced with a slug.
         !     Typically a value of 0.98 is used for βinit."
         !
         ! (b) IKH-based (thesis §3.2.3, page 49): the LASSI MSW becomes
         !     ill-posed when and only when κ < 0.  In that regime the
         !     Riemann problem has no solution and perturbations grow
         !     exponentially without bound (Inviscous Kelvin-Helmholtz
         !     instability), which is precisely the physical mechanism
         !     for slug initiation.  Convert the section to a slug as
         !     soon as κ < kappa_crit AND β > beta_ikh.  beta_ikh acts
         !     as a low-β safety floor — at very low holdup the
         !     stratified film is far from slug formation regardless of
         !     κ; default 0.5 covers all realistic IKH cases.
         if (p%kind == KIND_SECTION) then
            trigger_holdup = (p%beta > g%beta_init)
            trigger_ikh    = .false.
            if (p%beta > g%beta_ikh) then
               alpha_local = max(1.0_rk - p%beta, EPS_SMALL)
               U_g_local   = p%Ul
               rho_g_local = g%rho_g_ref
               if (associated(p%next)) then
                  if (p%next%kind == KIND_BUBBLE) then
                     U_g_local   = p%next%Ugs / alpha_local
                     rho_g_local = p%next%rhog
                  end if
               end if
               kappa_local = ikh_kappa(p%beta, g%D, g%phi, g%rho_l, rho_g_local, p%Ul, U_g_local)
               trigger_ikh = (kappa_local < g%kappa_crit)
            end if
            do_trigger = trigger_holdup .or. trigger_ikh
         else
            do_trigger = .false.
         end if
         if (do_trigger) then
...
!======================================================================
   subroutine slug_front_eat(g, dt)
      ...
      real(rk) :: remaining, z_front, z_cross, t_cross, U_F
      integer :: niter
      ...
      do while (associated(p))
         ...
         if (p%kind == KIND_SLUG) then
            ...
            U_F = front_speed(p%Um, sec_ahead%beta, sec_ahead%Ul)
            if (U_F <= EPS_SMALL) then
               p => p_next
               cycle
            end if
            z_front = bub_R%zR - U_F*remaining
            do while (remaining > EPS_TINY .and. niter < MAX_EAT_PER_STEP)
               ...
               U_F = front_speed(p%Um, sec_ahead%beta, sec_ahead%Ul)
               if (U_F <= EPS_SMALL) exit
               z_cross = sec_ahead%zR
               if (z_front >= z_cross - EPS_OVERSHOOT) then
                  t_cross = 0.0_rk
               else
                  t_cross = (z_cross - z_front)/U_F
               end if
               if (t_cross < remaining - EPS_TINY) then
                  remaining = remaining - max(t_cross, 0.0_rk)
                  p%zR = z_cross
...
!======================================================================
   subroutine slug_shed(g, dt)
      ...
      real(rk) :: eaten_rate, shed_rate, dLdt, L_predicted, L_min_die
      logical  :: paper_dies
      p => g%head
      do while (associated(p))
         p_next => p%next
         if (p%kind == KIND_SLUG) then
            ...
            if (associated(sec_R)) then
               ...
               L_predicted = L + dLdt*dt
               L_min_die   = 0.10_rk*g%TargetLength
               if (dLdt < 0.0_rk .and. L_predicted < L_min_die) paper_dies = .true.
            end if
