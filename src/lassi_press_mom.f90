...
contains
   ...
   subroutine pressure_momentum_step(g, dt, fp, eos, p_outlet, W_eff)
      ...
      type(object_t), pointer :: sec_L_gj, sec_R_gj
      real(rk) :: betaUl_L_gj, betaUl_R_gj, rho_L_gj, rho_R_gj
      ...
      max_dUm = huge(1.0_rk)
      picard_iter = 0
      max_picard_iter = max(1, g%picard_max_iter)
      ...
      do ib = 1, nb
         ...
         sec_L_gj => pm%sec_left(ib)%p
         sec_R_gj => pm%sec_right(ib)%p
         betaUl_L_gj = 0.0_rk
         betaUl_R_gj = 0.0_rk
         rho_L_gj = pm%rhog_old(ib)
         rho_R_gj = pm%rhog_old(ib)
         if (associated(sec_L_gj)) then
            select case (sec_L_gj%kind)
            case (KIND_SECTION)
               betaUl_L_gj = sec_L_gj%beta*sec_L_gj%Ul
               rho_L_gj = section_rhog_n(sec_L_gj)
            case (KIND_SLUG)
               betaUl_L_gj = sec_L_gj%Um
               rho_L_gj = pm%rhog_old(ib)
            end select
         end if
         if (associated(sec_R_gj)) then
            select case (sec_R_gj%kind)
            case (KIND_SECTION)
               betaUl_R_gj = sec_R_gj%beta*sec_R_gj%Ul
               rho_R_gj = section_rhog_n(sec_R_gj)
            case (KIND_SLUG)
               betaUl_R_gj = sec_R_gj%Um
               rho_R_gj = pm%rhog_old(ib)
            end select
         end if
         if (associated(sec_R_gj) .and. sec_R_gj%kind == KIND_SLUG) then
            pm%gJ(ib) = pm%pres(ib) + pm%zJ(ib)*( &
                pm%rhog_old(ib)*betaUl_L_gj &
               + pm%alpha_L(ib)*pm%U_b(ib)*(pm%rhog_old(ib) - rho_L_gj) )
         else if (associated(sec_L_gj) .and. sec_L_gj%kind == KIND_SLUG) then
            pm%gJ(ib) = pm%pres(ib) + pm%zJ(ib)*( &
               -pm%rhog_old(ib)*betaUl_R_gj &
               -pm%alpha_R(ib)*pm%U_b(ib)*(pm%rhog_old(ib) - rho_R_gj) )
         else
            pm%gJ(ib) = pm%pres(ib) + pm%zJ(ib)*( &
                pm%rhog_old(ib)*(betaUl_L_gj - betaUl_R_gj) &
               + pm%alpha_L(ib)*pm%U_b(ib)*(pm%rhog_old(ib) - rho_L_gj) &
               + pm%alpha_R(ib)*pm%U_b(ib)*(rho_R_gj - pm%rhog_old(ib)) )
         end if
      end do
      ...
      call pm_add_unit_correction_to_gJ(pm, nb, dt, A_pipe, gamma_eos, m_in, p_outlet, gas_track_old)
...
!======================================================================
   subroutine pm_compute_momentum_coeffs(g, pm, nb, dt, fp)
      ...
      do ib = 1, nb
         ...
         pm%bJ(ib) = -min(U_g_right - U_b_right, 0.0_rk)/pm%L_b(ib)
         pm%cJ(ib) =  max(pm%U_g(ib) - pm%U_b(ib), 0.0_rk)/pm%L_b(ib)
         ...
         pm%dJ(ib) = pm%m_old(ib)/dt &
                     + 0.125_rk*Si/max(A_pipe, EPS_SMALL)*lam_i*pm%rhog_old(ib)*pm%U_l(ib)*abs(pm%U_g(ib) - pm%U_l(ib)) &
                     - pm%rhog_old(ib)*pm%alpha_b(ib)*G_ACC*sin(g%phi)
      end do
   ...
   subroutine pm_block_solve(g, pm, nb, dt, fp, eos, p_outlet, W_eff_cap, m_in, iter_label)
      ...
      logical :: do_log, left_blocked, right_blocked
      ...
      do i = 1, nb
         ...
         left_blocked = associated(sec_L) .and. sec_L%kind == KIND_SLUG
         right_blocked = associated(sec_R) .and. sec_R%kind == KIND_SLUG
         if (right_blocked) then
            call add_coeff(A, row, m_col(i), 1.0_rk)
            rhs(row) = 0.0_rk
         else
            call add_coeff(A, row, m_col(i), pm%aJ(i))
            if (i > 1 .and. .not. left_blocked) then
               call add_coeff(A, row, m_col(i-1), -pm%cJ(i))
               rhs(row) = pm%dJ(i)
            else if (.not. left_blocked) then
         ...
         block
            ...
            do idx = 1, n_slug
               ...
               D_slug_val = diag_arr(idx)%rhoL_dt * diag_arr(idx)%Um_old &
                          ...
                          + diag_arr(idx)%intf
               write(g%pm_diag_lun, '(F11.6,1X,I3,1X,I8,26(1X,ES13.5E2))') &
                  ...
                  pL_val, pR_val, VgL_val, VgR_val, zJL_val, zJR_val, &
                  gJL_val, gJR_val, mL_val, mR_val
            end do
            flush(g%pm_diag_lun)
         end block
      end if
   ...
   subroutine slug_momentum_coeffs(g, slug, dt, fp, W_eff_cap, A_slug, D_slug0)
      ...
      real(rk) :: U_m_n, Lslug, L_total, L_bL, L_bR, lam_l, front_coeff
      real(rk) :: U_b_back, U_F, W_loc, beta_R, U_l_R, h_L, h_R, U_crit
      ...
      if (associated(sec_feed_R) .and. U_m_n > U_l_R .and. U_m_n > U_crit) then
         U_F = safe_div_ufront(U_m_n - beta_R*U_l_R, 1.0_rk - beta_R, U_m_n)
         front_coeff = g%rho_l*beta_R/max(1.0_rk - beta_R, EPS_SMALL)*(U_m_n - U_l_R)
      else
         U_F = U_m_n
         front_coeff = 0.0_rk
      end if
...
!======================================================================
   subroutine pm_update_slug_momentum(g, dt, fp, W_eff_cap)
      ...
      real(rk) :: pL, pR
      real(rk) :: lam_l
      real(rk) :: U_b_back, U_F, W_loc, U_crit
      real(rk) :: beta_R, U_l_R, h_L, h_R
      real(rk) :: A_slug, D_slug, front_coeff
      real(rk) :: A_pipe, alpha_R, gamma_geom, Sl, Sg, Si, A_g, Dg, Re_g, lam_i
      real(rk) :: rhog_R, Ug_R, interfacial_front
