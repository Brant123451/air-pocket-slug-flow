...
module lassi_io
   ...
   type :: case_params_t
      ...
      ! IKH-driven slug onset (thesis §3.2.3 page 49).  Section converts
      ! to slug if EITHER  β > β_init  (Hooke-style holdup threshold)
      ! OR  (β > beta_ikh  AND  IKH-coefficient κ < kappa_crit) — the
      ! latter triggers slug formation when the stratified flow has
      ! become linearly ill-posed (κ < 0).  Per the thesis, κ ≤ 0
      ! ⇔ Riemann problem has no solution ⇔ slugs form naturally.
      ! Defaults: trigger active for β > 0.5 with κ ≤ 0; set
      ! kappa_crit > 0 to disable the IKH branch.
      real(rk) :: beta_ikh     = 0.5_rk      ! lower-β floor for IKH branch
      real(rk) :: kappa_crit   = 0.0_rk      ! κ < kappa_crit  → IKH-onset trigger
...
contains
   ...
   subroutine assign_kv(cp, k, v)
      ...
      case('beta_ikh');      read(v,*) cp%beta_ikh
      case('kappa_crit');    read(v,*) cp%kappa_crit
