...
!
!   IKH ill-posed branch (κ ≤ 0, thesis page 49): the modified shallow
!   water has no Riemann solution.  Per thesis algorithm this is the
!   exact regime where slugs form.  We map it to a Saturated Shock-
!   Shock state (β_M = 1) so the cell remap (Eq. 3.28-3.29) drives the
!   adjacent sections towards β = 1 and slug_init converts them in the
!   next list-management pass.
!======================================================================
   subroutine solve_msw_riemann(beta_L, U_L, beta_R, U_R, kappa, st)
      ...
      if (kappa <= 0.0_rk) then
         ! IKH ill-posed branch (Eq. 3.12 violated): collapse to Saturated
         ! Shock-Shock so the section is captured as a slug.
         kp  = EPS_SMALL
         sqk = sqrt(kp)
         call fill_SSS(beta_L, U_L, beta_R, U_R, kp, sqk, st)
         return
      end if
      kp  = kappa
      sqk = sqrt(kp)
