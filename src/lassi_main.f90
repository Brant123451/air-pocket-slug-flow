...
program lassi_main
   ...
   else if (trim(cp%ic_mode) == 'localized_wave') then
      call apply_localized_wave_ic(g, cp)
   end if
...
contains
   ...
   subroutine apply_localized_wave_ic(g, cp)
      type(grid_t),        intent(inout) :: g
      type(case_params_t), intent(in)    :: cp
      type(object_t), pointer :: p
      real(rk) :: x_c, bump
      ...
      do while (associated(p))
         if (p%kind == KIND_SECTION) then
            x_c = p%zR - 0.5_rk*object_length(p)
            bump = cp%wave_amp*exp(-0.5_rk*((x_c - cp%wave_x0)/max(cp%wave_sigma, EPS_SMALL))**2)
            p%beta = max(0.05_rk, min(0.95_rk, cp%beta0 + bump))
            p%Ul = cp%usl/max(p%beta, EPS_SMALL)
            p%Ugs = cp%usg
         end if
         p => p%next
      end do
      ...
      do while (associated(p))
         if (p%kind == KIND_BUBBLE .and. associated(p%prev)) then
            if (p%prev%kind == KIND_SECTION) then
               p%beta = p%prev%beta
               p%Ul   = p%prev%Ul
               p%Ugs  = p%prev%Ugs
            end if
   ...
   end subroutine apply_localized_wave_ic
